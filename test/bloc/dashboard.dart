...
void main() {
  ...
  blocTest<DashboardBloc, DashboardState>(
    ...
    build: () {
      when(accountRepository.getAccounts(any))
          .thenAnswer((_) async => (null, [Account(id: 'acc1')]));
    ...
    expect: () => [
      ...
      DashboardState.loaded(
        ...
        bills: [],
        transactions: [Transaction(id: 'txn1')],
      ),
    ],
  );
  ...
  blocTest<DashboardBloc, DashboardState>(
    ...
    build: () => dashboardBloc,
    act: (bloc) => bloc.add(DashboardEvent.accountDataChange([Account(id: 'acc2')])),
    expect: () => [
      DashboardState.loaded(
  ...
  blocTest<DashboardBloc, DashboardState>(
    ...
    build: () => dashboardBloc,
    act: (bloc) => bloc.add(DashboardEvent.transactionDataChange([Transaction(id: 'txn2')])),
    expect: () => [
      DashboardState.loaded(
        accounts: [],
        bills: [],
  ...
  blocTest<DashboardBloc, DashboardState>(
    ...
    verify: (_) {
      ...
      verify(errorBloc.add(const ErrorEvent.showError('Transferring Funds', type: NotificationType.loading))).called(1);
      verify(accountRepository.makeTransfer(any, any, any, any)).called(1);
      verify(errorBloc.add(const ErrorEvent.showError('Transfer successful', type: NotificationType.notify))).called(1);
